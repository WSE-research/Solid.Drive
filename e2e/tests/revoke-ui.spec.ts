...
/**
 * The owner-side revoke flow, driven through the SharePanel UI.
 * Complements `revoke-keeps-visible.spec.ts` (which drives the ACL change
 * via fetch and asserts the requester side rendering).
 */
test("Peach revokes via the share panel and Parni loses access", async ({ browser, peach, parni }) => {
  ...
  await expect(before.page.locator("file-card").filter({ hasText: "Revoke Me" })).toBeVisible({ timeout: 30_000 });
  await before.close();
  ...
  await expect(after.page.getByRole("button", { name: /Request all Image/i })).toBeVisible({ timeout: 30_000 });
