...
/**
 * The full UI journey, end to end. Every interaction the user makes goes
 * through real DOM, including the contact-add and catalog-request flows
 * the focused tests bypass with direct fetch.
 *
 * Flow:
 ...
 *
 * Slower than the focused tests, but catches multi-step regressions
 * (state resets, cache busting, contact resolution, etc.).
 */
test("full UI journey: add contact → catalog approve → per-file request → type request", async ({ browser, peach, parni }) => {
  ...
  await expect(parniSession.page.locator("contact-row").filter({ hasText: "Peach" })).toBeVisible({ timeout: 15_000 });
  ...
  await expect(parniSession.page.getByText(/Also available|Auch verfügbar/i)).toBeVisible({ timeout: 30_000 });
  ...
  await parniSession.close();
  parniSession = await freshLogin(browser, parni);
  await expect(parniSession.page.locator("file-card").filter({ hasText: "Single Image" })).toBeVisible({ timeout: 30_000 });
  ...
  await expect(imageFolder.getByText(/Requested|Angefragt/).first()).toBeVisible({ timeout: 15_000 });
  ...
  parniSession = await freshLogin(browser, parni);
  await expect(parniSession.page.locator("file-card").filter({ hasText: "Single Image" })).toBeVisible({ timeout: 30_000 });
  await expect(parniSession.page.locator("file-card").filter({ hasText: "Bulk Image" })).toBeVisible({ timeout: 30_000 });
